...
export const RegUserInfo = ({navigation, route}: Navigation) => {
  ...
  const InfoSchema = Yup.object().shape({
    ...
    fatPercentage: Yup.number()
      .required('Указан некорректный процент жира')
      .min(1)
      .max(100),
  });
  ...
  return (
    <View style={[styles.mainView, {backgroundColor: colors.background}]}>
      <ScrollView
        ...
        />
        {errors.weight && (
          <AppText style={{fontSize: 18}}>{errors.weight}</AppText>
        )}
        ...
        />
        {errors.height && (
          <AppText style={{fontSize: 18}}>{errors.height}</AppText>
        )}
        ...
        {errors.fatPercentage && (
          <AppText style={{fontSize: 18, color: 'red'}}>
            {errors.fatPercentage}
          </AppText>
        )}
      </ScrollView>
